...
from helpers import base_path, clean_screen
...
def register(**user):
    ...
    pass_validation_mapper = {"upper": False, "lower": False, "digits": False, "special": False}
    ...
    for character in user['password']:
        if character in ascii_uppercase:
            ...
            pass_validation_mapper['digits'] = True
        elif character in punctuation:
            pass_validation_mapper['special'] = True

    if not all(pass_validation_mapper.values()):
        render_register_screen(error='Password must contain at least one upper case character, '
                                     'one lower case character, a digit and a special symbol!')
        return

    user.update({"products": []})
    with open(base_path + "/db/user_credentials_db.txt", "r+", newline='\n') as file:
...
def login(username, password):
    with open(base_path + "/db/user_credentials_db.txt") as file:
        data = file.readlines()
        for line in data:
            name, pwd = line.strip().split(', ')
            if username == name and password == pwd:
                with open(base_path + "/db/current_user.txt", "w") as f:
                    f.write(name)
